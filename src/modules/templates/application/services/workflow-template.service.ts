...
export class WorkflowTemplateService {
  ...
  private applySearchFilters(
    ...
    if (dto.categoryIds?.length) {
      query.innerJoin('template.categories', 'filterCategory').andWhere(
    ...
    if (dto.statuses?.length) {
      query.andWhere('template.status IN (:...statuses)', { statuses: dto.statuses });
    } else {
    ...
    if (dto.minRating !== undefined) {
      query.andWhere('template.averageRating >= :minRating', { minRating: dto.minRating });
    }

    if (dto.isFeatured !== undefined) {
      query.andWhere('template.isFeatured = :isFeatured', { isFeatured: dto.isFeatured });
    }

    if (dto.isOfficial !== undefined) {
      query.andWhere('template.isOfficial = :isOfficial', { isOfficial: dto.isOfficial });
    }

    if (dto.creatorId) {
      query.andWhere('template.creatorId = :creatorId', { creatorId: dto.creatorId });
    }

    if (dto.organizationId) {
      query.andWhere('template.organizationId = :organizationId', { organizationId: dto.organizationId });
    }
