...
import { Credential, CredentialType, ServiceType } from '../../domain/entities/credential.entity';
...
export class CredentialService {
  ...
  constructor(
    ...
    if (this.encryptionKey === 'default-key-change-in-production') {
      this.logger.warn('Using default encryption key. Change ENCRYPTION_KEY in production!');
    }
  }

  async createCredential(userId: string, dto: CreateCredentialDto): Promise<Credential> {
    ...
    if (dto.type === CredentialType.OAUTH2 && dto.credentialData.refreshToken) {
      ...
      if (dto.metadata?.expiresIn) {
        credential.expiresAt = new Date(Date.now() + (dto.metadata.expiresIn * 1000));
      }
    }
  ...
  async updateCredential(id: string, userId: string, dto: UpdateCredentialDto): Promise<Credential> {
    ...
    if (dto.metadata) credential.metadata = { ...credential.metadata, ...dto.metadata };

    if (dto.credentialData) {
      credential.encryptedData = this.encrypt(JSON.stringify(dto.credentialData));
      
      // Update OAuth2 specific fields
      if (credential.type === CredentialType.OAUTH2 && dto.credentialData.refreshToken) {
        credential.refreshToken = this.encrypt(dto.credentialData.refreshToken);
      }
    }
  ...
  async findByService(userId: string, service: ServiceType): Promise<Credential[]> {
    return this.credentialRepository.find({
  ...
  async getCredentialData(id: string, userId: string): Promise<CredentialData> {
    ...
    if (credential.type === CredentialType.OAUTH2 && this.needsTokenRefresh(credential)) {
    ...
    try {
      ...
      switch (credential.service) {
        ...
        case ServiceType.GMAIL:
          tokenResponse = await this.refreshGoogleToken(credentialData.clientId!, credentialData.clientSecret!, refreshToken);
          break;
        case ServiceType.GITHUB:
          tokenResponse = await this.refreshGitHubToken(credentialData.clientId!, credentialData.clientSecret!, refreshToken);
          break;
        case ServiceType.SLACK:
          tokenResponse = await this.refreshSlackToken(credentialData.clientId!, credentialData.clientSecret!, refreshToken);
          break;
        default:
          this.logger.warn(`Token refresh not implemented for service: ${credential.service}`);
          return;
      }
      ...
      if (tokenResponse.access_token) {
        ...
        credential.encryptedData = this.encrypt(JSON.stringify(newCredentialData));
        
        if (tokenResponse.expires_in) {
          credential.expiresAt = new Date(Date.now() + (tokenResponse.expires_in * 1000));
        }

        await this.credentialRepository.save(credential);
        this.logger.log(`Successfully refreshed OAuth2 token for credential ${credential.id}`);
      }
    } catch (error) {
      this.logger.error(`Failed to refresh OAuth2 token for credential ${credential.id}: ${error.message}`);
      throw new BadRequestException('Failed to refresh token');
    }
  }

  private async refreshGoogleToken(clientId: string, clientSecret: string, refreshToken: string): Promise<any> {
  ...
  private async refreshGitHubToken(clientId: string, clientSecret: string, refreshToken: string): Promise<any> {
    const response = await lastValueFrom(
  ...
  private async refreshSlackToken(clientId: string, clientSecret: string, refreshToken: string): Promise<any> {
    const response = await lastValueFrom(
