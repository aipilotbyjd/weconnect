...
export class ExecutionCleanupService {
  ...
  async cleanupStaleExecutions() {
    ...
    try {
      ...
      this.logger.log(`Found ${staleExecutions.length} stale executions to cleanup`);
  ...
  async cleanupOldExecutionLogs() {
    ...
    try {
      ...
      const result = await this.executionRepository.delete({
        status: In([ExecutionStatus.SUCCESS, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED]),
  ...
  async forceCleanupExecution(executionId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Force cleanup failed for execution ${executionId}:`, error);
      throw error;
    }
  }
  ...
  async getExecutionStatistics(): Promise<{
    ...
    const [running, pending, stale, total] = await Promise.all([
      this.executionRepository.count({ where: { status: ExecutionStatus.RUNNING } }),
      this.executionRepository.count({ where: { status: ExecutionStatus.PENDING } }),
