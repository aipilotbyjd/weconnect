...
export class OAuth2Service {
  ...
  private initializeProviders() {
    ...
    if (slackClientId && slackClientSecret) {
      this.providers['slack'] = {
        ...
        redirectUri:
          this.configService.get('SLACK_REDIRECT_URI') ||
    ...
    if (discordClientId && discordClientSecret) {
      this.providers['discord'] = {
        ...
        redirectUri:
          this.configService.get('DISCORD_REDIRECT_URI') ||
  ...
  private async exchangeCodeForProvider(
    code: string,
    provider: string,
  ): Promise<OAuth2Token> {
    const config = this.providers[provider];
    ...
    try {
      ...
      throw new UnauthorizedException(
        `Failed to exchange code for ${provider}: ${error.message}`,
      );
    }
  }
